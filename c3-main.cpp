...
/**
 * Implement Iterative Closest Point algorithm. This matches each source cloud point to its closest target (prior) cloud point
 * and returns the transformation matrix for aligning prior pose into current pose
 * 
 * @param target set of prior points
 * @param source set of current points to match with target
 * @param prior_pose initial pose where we measured prior points
 * @param interations iterations
 * @return transformation matrix for getting current pose from prior pose
 */
Eigen::Matrix4d ICP(PointCloudT::Ptr target, PointCloudT::Ptr source, Pose prior_pose, int iterations) {
  ...
  // Align source points to prior pose
  ...
  icp.setTransformationEpsilon(.000001);
  ...
  if (icp.hasConverged()) {
    ...
    transformation_matrix = transformation_matrix * prior_transform;
  }
...
void Accuate(ControlState response, cc::Vehicle::Control & state) {
  ...
  if (response.t > 0) {
    if (!state.reverse) {
      state.throttle = min(state.throttle + response.t, 1.0 f);
    } else {
      state.reverse = false;
      state.throttle = min(response.t, 1.0 f);
    }
  ...
  state.steer = min(max(state.steer + response.s, -1.0 f), 1.0 f);
...
int main() {
  ...
  while (!viewer -> wasStopped()) {
    while (new_scan) {
      std::this_thread::sleep_for(0.1 s);
      world.Tick(1 s);
    }
    if (refresh_view) {
    ...
    if (!new_scan) {
      ...
      // Student note: Use example from https://pointclouds.org/documentation/tutorials/voxel_grid.html

      pcl::VoxelGrid < PointT > vg;
      vg.setInputCloud(scanCloud);
      vg.setLeafSize(0.5 f, 0.5 f, 0.5 f); // resolution for Voxel dimensions
      vg.filter( * cloudFiltered);

      // TODO: Find pose transform by using ICP or NDT matching

      // Using ICP for now:
      Eigen::Matrix4d transform_matrix = ICP(mapCloud, cloudFiltered, pose, 100); // set iterations to 5
